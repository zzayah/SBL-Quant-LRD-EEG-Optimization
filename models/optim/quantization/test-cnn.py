...
from cnn import BATCH_SIZE, EPOCHS, LR, OUT_PATH, DynamicCNN, num_classes, x, y


DEVICE = "mps" if torch.backends.mps.is_available() else "cpu"
TRIALS_PATH = OUT_PATH
...
def load_best_trial(path: Path) -> dict:
    ...
    with path.open() as f:
        for line_number, line in enumerate(f, start=1):
            ...
            score = trial.get("optimization", {}).get("score", validation_accuracy)
...
def main() -> None:
    ...
    cnn = DynamicCNN(fixed_trial, x.shape[1], x.shape[2], num_classes)
